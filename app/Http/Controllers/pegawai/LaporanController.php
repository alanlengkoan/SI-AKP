    ...
    public function pegawai_get_data_dt(Request $request)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                return '
                    <a href="' . route('pegawai.pegawai.det', my_encrypt($row->id_pegawai)) . '" class="btn btn-info btn-sm"><i class="fa fa-info-circle"></i>&nbsp;Detail</a>&nbsp;
                ';
            })
            ->make(true);
    }
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                if ($row->toPegawaiPangkat->count() > 1) {
                    return '
                            <a href="' . route('pegawai.pegawai.print', my_encrypt($row->id_pegawai)) . '" class="btn btn-sm btn-success" target="_blank"><i class="fa fa-print"></i>&nbsp;Cetak</a>&nbsp;
                            <a href="' . route('pegawai.pegawai.det', my_encrypt($row->id_pegawai)) . '" class="btn btn-info btn-sm"><i class="fa fa-info-circle"></i>&nbsp;Detail</a>
                        ';
                } else {
                    return '<a href="' . route('pegawai.pegawai.det', my_encrypt($row->id_pegawai)) . '" class="btn btn-info btn-sm"><i class="fa fa-info-circle"></i>&nbsp;Detail</a>';
                }
            })
            ->make(true);
    }
